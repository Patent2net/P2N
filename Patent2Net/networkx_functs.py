...
def ChargeAffectation(fichier):
    ...
    for ligne in fichier.readlines():
        if ligne.count('-'): #la ligne contient un -
            try:
                tempo = ligne[0:ligne.index('-')-1]
                tempo = tempo.title()
                tempo = tempo.split(' ')
                
                Affec.add(tempo[0]+tempo[1][:1])
                
            except:
                pass
    return Affec
    #print I3M
def ChargeVille(fichier):
    fichier = open(fichier)
    Ville = dict()
    for ligne in fichier.readlines():
        if ligne.count('-'): #la ligne contient un -
            try:
                tempo = ligne[0:ligne.index('-')-1]
                if ligne.lower().count('toulon') >0:
                    ville = 'Toulon'
                elif ligne.lower().count('nice') >0:
                    ville = 'Nice'
                else:
                    ville = '???'
                tempo = tempo.title()
...
def main():
    ...
    path = 'C:/wamp/www/I3M2/'
    inputjsonfile = 'new_json.json'
    I3M  = ChargeAffectation('ChercheursI3M.txt')
    I3MAssoc = ChargeAffectation('ChercheursAssocies.txt')
    
    
            
    
    
    edgesfile = 'appariement.txt' #'pairs_by_label_clean.edgelist'#
...
#        print "highest eigenvector centrality nodes:", key, val
    ...
    #small_filename = "I3Mrest_test.json"
    #save_to_jsonfile( path+small_filename, small_graph )
    
    nx.write_gexf(g, path+'I3M.gexf')
...
networks, Vincent D Blondel, Jean-Loup Guillaume, Renaud Lambiotte, Renaud Lefebvre,
